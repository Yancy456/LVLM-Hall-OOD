...
class EntailmentDeberta(BaseEntailment):
    ...
    def check_implication(self, text1, text2, *args, **kwargs):
        inputs = self.tokenizer(
            text1, text2, return_tensors="pt", max_length=200, truncation=True, padding=True).to(DEVICE)
...
def get_semantic_ids(strings_list, model, strict_entailment=False, example=None):
    ...
    def are_equivalent(text1, text2):
        '''0 typically stands for contradiction (text1 does not imply text2).
           1 stands for neutral (no clear implication).
           2 stands for entailment (text1 implies text2).
        '''
