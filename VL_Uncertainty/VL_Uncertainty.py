...
def infer_single_sample(args, lvlm, sample, is_sampling, llm, log_dict):
    ...
    if not is_sampling:
        ...
        if BENCHMARK_TYPE[args.benchmark] == 'MULTI_CHOICE':
            ...
            llm_ans_check = llm.generate(None,
                                         question,
                                         0.1
                                         )
...
def uncertainty_estimation(args, idx, llm, log_dict):
    ans_sampling_list = log_dict[idx]['ans_sampling_list']
    ans_cluster_idx = []

    ans_cluster_idx = [-1] * len(ans_sampling_list)
    cur_cluster_idx = 0
    log_dict[idx]['entailment'] = {}
    ...
    log_dict[idx]['cluster_dis'] = cluster_dis
...
def hallucination_detection(args, idx, log_dict):
    flag_predict_hallucination = log_dict[idx
    ...
    flag_detection_correct = (log_dict[idx]['flag_ans_correct'] and not flag_predict_hallucination) or (
        not log_dict[idx]['flag_ans_correct'] and flag_predict_hallucination)
...
def handle_batch(args, lvlm, benchmark, llm):
    ...
    log_dict['Hallucination detection accuracy'] = (
        cnt_correct_detection / total) * 100
