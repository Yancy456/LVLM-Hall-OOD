...
class LLaVA(LargeMultimodalModel):
    def __init__(self, args):
        ...
        self.tokenizer, self.model, self.image_processor, self.context_len = load_pretrained_model(
            args.model_path, model_base, model_name, load_8bit, load_4bit)
        print(model_name)
    ...
    def forward(self, image, prompt):
        ...
        response = self.tokenizer.decode(
            outputs[0, input_ids.shape[1]:]).strip()
        return response[:-4]
    ...
    def forward_with_probs(self, image, prompt):
        ...
        if isinstance(outputs, BeamSearchDecoderOnlyOutput):
            ...
            probs = [nn.functional.softmax(
                next_token_scores, dim=-1) for next_token_scores in outputs['scores']]
...
#         output_probs = [probs[i][output_ids[i]] for i in range(len(probs))]
        ...
        return response, output_ids, logits, probs, outputs.hidden_states
