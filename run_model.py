...
def get_model_output(args, data, model, processor, extra_keys, answers_file):
    ''''
    ...
    for ins in tqdm(data):
        ...
        if 'img_path' in ins:
            ...
            results = llm_generation.multiple_generation(
                **inputs)
        else:
            image = None
            conversation = [
            ...
            # prompt = processor.apply_chat_template(
            #                conversation, add_generation_prompt=True).to(0)
        ...
        '''
        judge = AnswerJudge(args.dataset, args.prompt)

        ins['most_likely']['correctness'] = judge.check_answer(ins)
...
def main(args):
    ...
    results = get_model_output(
        args, data, model, processor, extra_keys, args.answers_file)


if __name__ == "__main__":
    # arguments = Arguments(default_config='./configs/jailbreak.yaml')
    arguments = Arguments(default_config='./configs/triviaQA.yaml')
